...
class UITarsSubagent:
    ...
    def run(self, subtask, start_url=None, *, session_ref=None, close_on_finish=False,
            task=None, site=None, cancel_event=None, max_runtime_seconds=None):
        ...
        browser = SteelBrowser(width=self.width, height=self.height, session_ref=session_ref,
                               close_on_finish=close_on_finish, task=task, site=site)
        ...
        try:
            ...
            if start_url:
                ...
                shot = browser.screenshot_b64()
                self._last_obs = self._save_screenshot(shot, step)
            instruction = f"{subtask.rstrip()} Write the finished() content in English."
            if task:
                instruction += "\nFor a native dropdown, click to focus it and use select(value='...') to choose its value. type() replaces the focused field; do not append a newline. Only click, type, select, open_url, wait, zoom and finished are available for this task."
            messages = [{"role": "user", "content": [
            ...
            while step < self.max_steps:
                if cancel_event is not None and cancel_event.is_set():
                    result = {"success": False, "summary": "Visual execution cancelled.", "data": None}
                    break
                if max_runtime_seconds is not None and time.time() - t0 >= max_runtime_seconds:
                    result = {"success": False, "summary": "Visual runtime budget exhausted.", "data": None}
                    break
                self._trim_images(messages)
                reply = self._chat(messages)
                if (cancel_event is not None and cancel_event.is_set()) or (
                    max_runtime_seconds is not None and time.time() - t0 >= max_runtime_seconds
                ):
                    result = {"success": False, "summary": "Visual execution cancelled.", "data": None}
                    break
                ...
                try:
                    ...
                    if name == "finished":
                        ...
                        if task and submit:
                            raise ValueError("Use the configured search button after typing; implicit Enter is unsupported.")
                        _, shot = browser.execute("type", {"text": content.rstrip("\n")})
                        if submit:
                            _, shot = browser.execute("key", {"text": "Return"})
                        entry["action"]["input"] = {"text": content}
                    elif name == "select" and task:
                        content = str(kw.get("value", ""))
                        _, shot = browser.execute("select", {"text": content})
                        entry["action"]["input"] = {"text": content}
                    elif name == "scroll":
                        ...
                        if task:
                            _, shot = browser.execute("wait", {"duration": 1})
                        else:
                            time.sleep(5)
                            shot = browser.screenshot_b64()
                    else:
                    ...
                    entry["error"] = "Visual action failed." if task else str(e)
                    feedback.append("action failed" if task else f"action failed: {e}")
                    action_failures_in_a_row += 1
                    err_l = str(e).lower()
                    if "session" in err_l and ("404" in err_l or "not found" in err_l or "expired" in err_l):
                        self._log(entry)
                        result = {"success": False, "summary": "Browser session lost." if task else f"aborted: browser session lost ({e})", "data": None}
                        break
                    if action_failures_in_a_row >= 3:
                        self._log(entry)
                        result = {"success": False,
                                  "summary": "Three consecutive visual action failures." if task else f"aborted: 3 consecutive action failures (last: {e})", "data": None}
                        break
                else:
                    action_failures_in_a_row = 0
                ...
                if task:
                    entry["ghost_action"] = browser.ghost_action
                self._log(entry)
                step += 1
            ...
            result["session_disposition"] = browser.stop()
            if self._trace:
                self._trace.close()
