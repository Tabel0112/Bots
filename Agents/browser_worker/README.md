...
## Ghost memory and automatic visual fallback

Set `GHOST_API_URL` to connect `Worker.run` and the worker API to Ghost. The worker tries
DOM exploration or saved semantic replay first. If DOM cannot interpret the page, the
existing UI-TARS worker continues in the same Steel session using the remaining budget.
Canvas without readable DOM results requests visual help after the initial inspection.
...
only when that request points to a hosted configured site. Use `--demo` for the local
no-key fixture.
The sample URLs need a hosted configured site for Steel. The no-key connected demo is
`python -m Agents.browser_worker.ghost_cli --demo` (catalog on 8766, Ghost on 8765).

For an explicit live Steel + GPT attempt, supply a request whose `site_id`, start URL,
domains, paths and controls point to an operator-approved hosted read-only site:

```sh
OPENAI_MODEL=gpt-5.4 OPENAI_REASONING_EFFORT=medium \
python -m Agents.browser_worker.ghost_cli --live-steel \
  --request path/to/hosted-read-only-request.json
```

`--live-steel` never starts the localhost fixture and requires `STEEL_API_KEY`,
`OPENAI_API_KEY`, `WORKER_BROWSER=steel`, and a non-loopback request URL. It prints
sanitized phase events to stderr while the authoritative report remains JSON on stdout.
The existing `session_viewer_url` is available in the standalone visual worker, but the
DOM worker does not yet expose a browser viewport URL or screenshots; Steel provider
viewer availability must not be inferred from the CDP connection URL. Use
`python -m Agents.browser_worker.demo.steel_sanity --use-environment` first to verify Steel
create/connect/observe/release without making an OpenAI call.

Setup, qualification/reuse commands, Python usage, message examples and failure cases
are in the [worker connection guide](../ghostapi/INTEGRATION.md). New files:
`ghost.py` coordinates memory/session ownership, `ghost_adapter.py` compiles and replays
semantic steps, and `ghost_cli.py` exposes run/qualification commands. Dependencies are
the existing browser stack plus HTTPX; the visual extra adds Pillow. `STEEL_API_KEY`,
`OPENAI_API_KEY`, `UITARS_BASE_URL` and `WORKER_SITES_FILE` configure the live path.

Output adds `report.ghost` (mode, candidate/run identity, memory errors) and
`report.visual_report` when fallback runs. A successful local search can return
`ghost.mode: exploration` and a candidate, followed by `ghost.mode: reuse` with zero
model calls after qualification. An unavailable Ghost service records `GHOST_UNAVAILABLE`
while permitting ordinary exploration. A visual answer without independently verifiable
structured records remains `inconclusive`; no candidate is saved. Quarantine/repair and
credentialed Steel/model verification remain pending. The scoped pytest command below
now includes worker–Ghost integration tests.
...
```powershell
.\.venv\Scripts\python.exe -m Agents.browser_worker.demo.run --executable "C:\Program Files\Google\Chrome\Application\chrome.exe"
```
...
```powershell
.\.venv\Scripts\python.exe -m uvicorn Agents.browser_worker.main:app --host 127.0.0.1 --port 8000
```
...
`python -m Agents.browser_worker.demo.run --live-model` is an alternative single-command GPT + local browser smoke test. It starts and stops the local fixture automatically. It still requires `OPENAI_API_KEY` and a local browser.
...
load_dotenv()
request = json.loads(Path("Agents/browser_worker/examples/subtask.json").read_text())
...
Every request includes IDs, version, objective, `site_id`, an operation, session ownership, parameters, known output schema ID, explicit success checks, allowed actions/domains, budgets, and required evidence. `search_extract` uses a trusted site configuration exactly as before. `open_search` accepts `site_id: open:<hostname>`, a required string `query`, optional string/number filters, `output_schema_id: open-records.v1`, and `expected_record_shape`; see [open_search.json](examples/open_search.json). It synthesizes narrow authority for that hostname and `www.<hostname>` rather than adding the site to the trusted registry. `start_url` defaults to `https://<hostname>/`. It permits only generic observed text/search/combobox input and button/link/checkbox controls, while the existing GET/HEAD-only traffic interception, password exclusion, domain/path guards, action budgets, and cancellation remain enforced. `start_url` can otherwise be omitted; a newly created browser uses the configured URL and an attached browser uses its current page. Prerequisites are optional but must all be `succeeded` if supplied. All supported evidence types are always returned, even if the request asks for fewer.
...
For `open_search`, independent verification runs only `fresh_extraction`, `current_run`, the synthesized `output_schema`, `field_provenance`, `approved_links`, and `query_applied`. The last check requires a succeeded `fill` whose value and value origin match the request's `query`. Selector coverage, configured result readiness, non-query parameter evidence, and configured/request conditions are recorded as `"<check>: not applicable on an open site"` limitations instead of passing. A zero-record success additionally needs visible text matching the bounded heuristic “no results” or “0 results”; that heuristic marks the open result state but is not promoted to a separate passing check. Model-selected containers and fields remain observation-bound, and every extracted field retains its element provenance. These checks do not establish exhaustive search, ranking quality, filter application beyond the query, or cross-page completeness.
...
The Ghost connection now consumes this `needs_visual` report and attaches UI-TARS to
the retained Steel session. Automated connection tests verify routing and lifecycle with
simulated Steel/model calls. Thomas and Tianqi's live receiver check and further adapter
convergence remain pending; see the connection guide above.
...
Known limits: configured sites and bounded hostname-scoped `open_search` only; approximate DOM accessibility names; at most 220 observed elements and 12,000 visible-text characters; truncated observations cannot pass verification. Iframes, shadow DOM, canvas contents, screenshots, JSON-LD extraction, and arbitrary source-code interpretation are not implemented. `needs_visual` is a handoff, not an embedded VLM. Login/account actions, POST search endpoints, downloads, arbitrary code, and transactional submissions are unsupported. Open search does not prove ranking, non-query filters, pagination or complete coverage. The Steel create/connect/observe/release sanity check passed at the recorded checkpoint; live GPT, hosted open-site tasks and receiver integration remain unverified.
