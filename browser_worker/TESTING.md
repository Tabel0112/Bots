...
```powershell
Get-Location
Test-Path .\browser_worker\README.md
...
.\.venv\Scripts\python.exe -c "import browser_worker; print(browser_worker.__version__)"
```
...
```powershell
python -m venv .venv
.\.venv\Scripts\python.exe -m pip install -r browser_worker\requirements-tested.txt
...
```powershell
.\.venv\Scripts\python.exe -m browser_worker.demo.run `
...
Leave `STEEL_API_KEY` empty for this test. `.env` is ignored by Git. Do not put credentials into `browser_worker/examples/subtask.json` or a command-line argument.

Run the live-model demo:

```powershell
.\.venv\Scripts\python.exe -m browser_worker.demo.run --live-model
```
...
```powershell
Set-Location "C:\TianqiUser\BattleOfSchools Hackathon\Bots"
.\.venv\Scripts\python.exe -m uvicorn browser_worker.main:app `
...
```powershell
Copy-Item .\browser_worker\sites.json .\browser_worker\sites.steel.json
notepad .\browser_worker\sites.steel.json
```
...
```powershell
Copy-Item .\browser_worker\examples\subtask.json .\browser_worker\examples\subtask.steel.json
notepad .\browser_worker\examples\subtask.steel.json
```
...
WORKER_SITES_FILE=browser_worker/sites.steel.json
```
...
```powershell
$steelBody = Get-Content -Raw .\browser_worker\examples\subtask.steel.json
...
```powershell
$request = Get-Content -Raw .\browser_worker\examples\subtask.json | ConvertFrom-Json
...
$request | ConvertTo-Json -Depth 20 | Set-Content .\browser_worker\examples\subtask.custom.json
```

Submit it:

```powershell
$customBody = Get-Content -Raw .\browser_worker\examples\subtask.custom.json
