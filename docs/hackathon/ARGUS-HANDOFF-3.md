...
Read AGENTS.md, docs/hackathon/ARGUS-HANDOFF-3.md (especially the section on what moderator/ is and is not), docs/hackathon/ARGUS.md (report intake decision rules, hard limits on the moderator, handoff callables), argus/interfaces.py (Moderator, ProgressObserver), argus/contracts.py (ModeratorDecision, MODERATOR_DECISIONS, AnswerSelection, Claim, Note, WorkerReport), argus/model_client.py, argus/fakes.py (StubModerator as the behavioural baseline, FakeModelClient), argus/controller.py (how decisions are executed and capped), and moderator/moderator.py (_synthesize, _aggregate, _monitor_trace) plus moderator/README.md.
...
- synthesize(interpreted, records, validation, evidence, failures): apply explicit criteria deterministically (filter, rank, limit, as StubModerator does), preserving each record's original input index. Return `AnswerSelection(record_indices, claims, notes)`, never a `FinalAnswer`: `record_indices` selects the controller-provided validated records in output order; each `Claim(record_index, fields)` indexes that selected order and names fields to state; typed `Note` entries may use only the fixed contract kinds and controller-valid subjects. Return no records, evidence refs, text, reasons or unverified strings. The controller derives the records and renders all user-facing lines. A model call is optional and may only choose ordering and fields; Thomas's prose synthesis from moderator/moderator.py cannot be published. If reused, parse it into this selection and discard the prose. On refusal or invalid output, return the deterministic selection and a `synthesis_fallback` note with one of the contract's allowed fixed subjects.
...
- Every decision string must come from contracts.MODERATOR_DECISIONS. The adapter never changes budgets, never touches sessions, never writes results, and never produces user-facing prose.

Tests use FakeModelClient (no network): each deterministic assess branch; model accept and model verify; refusal degrades to verify; reconcile with and without conflicts; synthesize applies criteria and returns structured claims over the selected records with typed notes for criteria it could not apply, and never returns text; fallback on invalid model output; observe_progress flag, stop and continue; isinstance checks for both protocols; a test that runs the full Controller with this moderator, FakeToolbox, FakeGhost and a JsonStore on the registry fixture and on the salary chain fixture with FakePlannerClient, asserting the same outcomes the StubModerator end-to-end tests assert.
