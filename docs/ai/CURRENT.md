...
The ARGUS base is committed on `feat/argus-controller-base` at `7641f87`; phase 0b and the phase 1b work (prompts E0, E, F and 2b) are local and uncommitted. Open-world navigation now runs end to end offline: two interpreter tiers with verified spans, gate rules S1-S5 with the approved ranking clarification, one `ModelClient` call for open scheduling only (`OPENAI_API_KEY`, `OPENAI_BASE_URL`, `ARGUS_MODEL`), `inputs_from` transfer between dependent subtasks, generic validation through a `report_context` keyword on `Ghost.validate`, criteria applied at synthesis, and a `--plan-fixture` flag that injects a Plan JSON through `FakePlannerClient` for explicit offline planning. This step also fixed reconciliation, which duplicated every job in a search-then-open_results chain, and made the controller carry the gate's own rejection codes (`DOMAIN_NOT_ALLOWED`, `ACTION_CLASS_NOT_ALLOWED`).

Checks actually run at this checkpoint: `python3 -m unittest discover -s argus/tests -v` passed **419 tests in 0.880s** (Python 3.13.5); `git diff --check` was clean; the open fixture ended `needs_input` at gate S4 with no session opened (exit 1); the clarified salary request planned by `plan_open_chain.json` ended `succeeded` (exit 0) with four records unique by url, remote-only, salary descending, each claim citing its own observation; the registry fixture run was unchanged. `openai` 2.14.0 is installed locally but **no live model call has been made**, and no browser, Steel, VLM, DNS or Ghost check was run. Still owed by the live toolbox: DNS/connection/redirect/rebinding and request enforcement, execution-time blocking of login, payment and submission actions, and four-slot VLM arbitration across runs and callers. Sting has not seen the `report_context` keyword. Actual behavior and limits are in [argus/README.md](../../argus/README.md) and the ARGUS-1 phase 1b block in [TEAM.md](TEAM.md); Abel reviews and commits.
