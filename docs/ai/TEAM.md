...
| ARGUS-1 | Task plan, routing and controller boundary | Abel | Building (re-review requested) | Tianqi's PR #10 review found four P1 provenance/budget/identity holes plus Windows write, encoding, packaging and CI gaps; all fixed with reproducing tests (claims and records bound to run evidence and validated records; reconciled records must be accepted records; hard max_seconds cutoff with session close; report request_id and session handle enforced; retrying atomic writes; UTF-8; argus in wheel and CI). Awaiting Tianqi's re-review before Ready to connect | PR #10; 451 offline tests OK on macOS without PYTHONUTF8; CI matrix added (ubuntu, windows x 3.12, 3.13) |
