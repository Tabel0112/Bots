...
| ARGUS-3 | Phase 3 connections: DOM adapter, Ghost bridge, moderator adapter, connected runtime, first connected run | Abel (took over 2026-09-13; Sting, Tianqi, Thomas verify their boundary tomorrow) | Building — INT-2 passed locally | Packages 1, 2, 3, 5 of [ARGUS-3-PLAN](../hackathon/ARGUS-3-PLAN.md) merged on `feat/argus-3`; P3-MOD (Thomas's module adapted) pending; ARGUS Steel session manager deferred | `argus/adapters/`, `argus/runtime.py`; 6 connected runs recorded below |
...
| INT-2 | First connected request with verified result | Abel ran; Sting/Tianqi/Thomas verify | **Passed locally 2026-09-13** (local Chrome, real gpt-5.6-sol interpreter, real gpt-5.4 DOM worker, Ghost API, deterministic stub moderator) | Rerun with the adapted moderator module and on Steel with a hosted catalog; teammates sign their receiver checks | Runs `run-f8a2282e12`, `run-ce55851bc2` in the ARGUS-3 block below |
| INT-3 | Learning, qualification, reuse and repair connections | Abel ran the first slice; Sting verifies | **Qualify + reuse passed locally 2026-09-13** (S3, S4); repair (S8/S9) not started | Controlled site v2, quarantine and repair remain; qualification job needs an API route (today: script) | `demo-catalog.search_extract` v4 qualified; reuse runs `run-194924dbdb`, `run-7763770673` |
...
### OW-1 open-world read-only search — 2026-09-13 (Codex from Abel's brief; reviewed, fixed and run live by Abel)

```text
Task ID / date / status: OW-1 / 2026-09-13 / Building — first live open-world run passed on Steel (Wikipedia); real-estate portal blocked
Artifact, branch/revision or local files: feat/argus-3-followups — Agents/browser_worker (open_search operation, synthesized open SiteConfig, generic verifier checks), argus/site_suggestion.py, argus/runtime.py (connected_interpret, open_plan), argus/adapters/worker_contracts.py (open_search mapping), frontend (site choice, limitations), tests argus/tests/test_{site_suggestion,open_plan}.py, tests/browser_worker/test_open_search.py
Module README / usage instructions: Agents/browser_worker/README.md (open_search), argus/README.md, frontend/README.md
Entry point + environment names: same connected runtime; a request naming no site triggers one ARGUS_MODEL call that proposes public sites, filtered by the domain policy, recorded as parameter site_choice (source "suggested")
Contract version + input/output/failure examples: worker operation open_search on site_id open:<host>; query = the request's filter criteria joined ("condos for sale in Toronto"); checks that need a site config are reported as limitations, never as passes
Checks run + result (offline): unittest argus/tests 585+ OK; worker pytest 99 passed; both Ruff configs; node --check.
Checks run + result (live, Steel):
  "Give me the best condos for sale in Toronto with their prices and links." — ARGUS chose realtor.ca; the gate first asked what "best" means (rule S4; its examples were job-specific and are now generic); after "lowest listed price" the worker opened https://www.realtor.ca/ and reported "site access" blocked (run-e073f52230). realtor.ca returns HTTP 403 to a plain request from this network: anti-bot, not our code.
  "On en.wikipedia.org, search for Toronto condominium towers ..." — first run (run-2ba4bd41ea) succeeded mechanically but the worker typed the goal sentence into the search box (results about search engines, validation passed): fixed, the query now comes from the filter criteria. Rerun run-f29275df0b: SUCCEEDED — five relevant articles (List of tallest buildings in Toronto, Toronto-Dominion Centre, World Trade Centre Toronto, Ritz-Carlton Toronto, M City Condominiums) with provenance, validation passed, 91.7 s, 6 browser actions, 9 worker model calls; limitations list the filter/rank criteria the open site could not verify.
  Fixes found live: Ghost ids cannot contain ':' (open:host -> open-host); open sites allow non-sensitive GET query keys (site searches redirect to ?search=...); DOM toolbox retries a SESSION_UNAVAILABLE start once after 5 s (ARGUS_SESSION_RETRY, ARGUS_SESSION_RETRY_DELAY) because Steel intermittently rejects a create right after a release.
Morning dry run 2026-09-14 from the runbook commands: all four prompts passed (explore run-50781f7072, reuse run-a530280640 replaying v3 with 0 model calls, two categories run-c9bff37cff 40 checks, Wikipedia run-e2d375f333 five correct articles). Finding for Tianqi/Sting: the open-world change altered the worker's SiteConfig shape, which is part of Ghost's compatibility key, so v1 (qualified 2026-09-13) no longer matched and the first reuse attempt explored; v3 was qualified with docs/hackathon/scripts/qualify_hosted.py. Any worker or site-config change requires re-qualification; consider hashing only the fields replay depends on.
Open issue / needed from / next action: pagination and multi-step (brief 4) not started; visual fallback unavailable (no UI-TARS endpoint); the moderator's model synthesis fell back to the deterministic path; Tianqi to review the worker changes (open_search, settle, query keys, ghost id); realtor.ca-class sites need Steel's proxy/captcha options and are not demo-safe.
Receiver + connection check/result: none marked Integrated; Tianqi (worker), Sting (registry ids for open sites), Thomas (moderator) verify tomorrow.
```

### ARGUS-3 connected runtime, INT-2 and INT-3 first slice — 2026-09-13 (Abel)

```text
Task ID / date / status: ARGUS-3 + INT-2 + INT-3 slice / 2026-09-13 / Building; INT-2 and qualify/reuse passed locally
Artifact, branch/revision or local files: feat/argus-3 — argus/adapters/{worker_contracts,dom_toolbox,ghost_bridge}.py, argus/runtime.py, argus/api/{app,service}.py, argus/controller.py (report_context for registry subtasks), frontend/dist/*, tests argus/tests/test_{worker_contracts,dom_toolbox,ghost_bridge,runtime}.py; plan docs/hackathon/ARGUS-3-PLAN.md
Module README / usage instructions: argus/README.md (Mission Control demo, runtime modes, startup commands), frontend/README.md
Entry point + environment names: ARGUS_RUNTIME=connected python -m argus.api; OPENAI_API_KEY, ARGUS_MODEL=gpt-5.6-sol, OPENAI_MODEL=gpt-5.4, OPENAI_REASONING_EFFORT=medium, GHOST_API_URL=http://127.0.0.1:8766, WORKER_BROWSER=local, WORKER_BROWSER_EXECUTABLE, ARGUS_MODERATOR=stub (until the module lands), ARGUS_STORE
Contract version + input/output/failure examples: ARGUS 0.5 <-> worker SubtaskRequest/SubtaskReport 0.2 via worker_contracts (operation search_products -> search_extract; worker validation/ghost blocks kept out of the ARGUS report); Ghost is worker-resolved (bridge previews via POST /v1/workflows/lookup, never writes); sessions worker-owned; unknown operations/sites fail before any browser work
Checks run + result (offline): unittest argus/tests 536 OK; ruff check/format clean; node --check; git diff --check.
Checks run + result (live, local Chrome, real models, Ghost API 8766, catalog 8765, stub moderator):
  run-f8a2282e12 "headphones under 150 USD, five cheapest": succeeded, validation passed (21 checks), 28 events, 32.1 s total, worker 19.1 s / 4 browser actions / 5 gpt-5.4 calls, 2 records (Studio 129, Travel 79 USD), candidate demo-catalog.search_extract v1 saved by the worker and referenced by ARGUS.
  run-ce55851bc2 "headphones under 150 and keyboards under 100": succeeded, 42 checks passed, 42 events, 44.6 s, two subtasks run serially (same site group), reconcile merged; keyboards subtask returned an evidenced empty result (the catalog item is "Mechanical keyboard" 89 USD; the interpreter passed the plural "keyboards" which the catalog search does not match — demo prompts must say "keyboard"); candidate v2.
  Qualification of v1 by script (GhostWorkflow.qualify, three changed inputs, worker-owned local sessions): 3 replays succeeded with 0 model calls (1.0–3.6 s each), incl. an evidenced empty result (keyboard <= 50); v1 qualified.
  run-6df364bf20 "headphones under 100": succeeded but explored again (5 model calls) — reuse failed because worker_contracts translated max_results into a max_records success condition, and success conditions are part of Ghost's compatibility key. Fixed: max_results is now a controller-side limit, not a worker condition.
  run-f3f65af59d explore -> candidate v4; qualification of v4: 3 replays, 0 model calls; v4 qualified.
  run-194924dbdb "headphones under 100": REUSE — worker_model ghost-semantic-replay, 0 model calls, worker 1.4 s, 11.4 s total, validation passed, 1 record (Travel 79). ARGUS stage-4 preview still said "no compatible qualified workflow" because the bridge opened the default SQLite file, not the Ghost API's database.
  Fixed: the preview now POSTs the worker-shaped lookup (compatibility key included) to GHOST_API_URL. run-7763770673: reuse again, 0 model calls, 13.6 s, match reason "qualified workflow demo-catalog.search_extract v4 compatible; the worker will replay it" shown in the UI inspector.
  Ghost registry after the night: v1 qualified, v2 candidate, v3 candidate, v4 qualified. No server errors in any run.
  With the adapted moderator module (ARGUS_MODERATOR=module, gpt-5.6-sol): run-5c84567917 failed — the model asked to "verify" because its prompt lacked the request parameters, and the controller's verify step needs the deferred session manager. Fixed: the prompt now carries request parameters, actions and evidence, and with verification_available=False a model "verify" is recorded as a deferred verification and accepted for the independent validation stage (a model refusal still returns verify). run-71274a4f9c failed — the model treated the interpreter's max_results=1 as a worker obligation; fixed by presenting the result limit as controller-applied. run-9ae68d58be "headphones under 150 USD and a keyboard under 100 USD, cheapest in each category": SUCCEEDED with the module — both subtasks replayed qualified v4 (0 worker model calls, 1.6 s and 1.0 s), moderator accepted each with grounded reasons (4 moderator model calls), reconcile merged 3 records, 40 checks passed, 29.5 s total; published Travel headphones 79, Mechanical keyboard 89, Studio headphones 129 USD. The model's field choice dropped the url from the rendered lines; fixed by making url a mandatory claim field (not yet rerun live). Records carry no category field, so the cheapest-per-category rule and the interpreter's max_results did not trim the third record.
Follow-ups landed later the same night (Codex, reviewed by Abel; on feat/argus-3-followups): P3-SESSION `argus/adapters/steel_sessions.py` (bounded Steel session manager with screenshot evidence, 8 tests; not yet wired into the runtime), P3-LEARN `argus/api/qualification.py` (POST /api/workflows/{skill}/versions/{v}/qualifications runs GhostWorkflow.qualify as a job, GET job status, GET /api/workflows proxy; 7 tests), and Mission Control replay/exploration badges plus a Ghost Library view. Suite 573 OK. Hosted catalog: https://tabel0112.github.io/Bots-Hosting/catalog.html with `demo-site/sites.hosted.json`; the first Steel attempt (run-8a8d6d4c8b) failed in 0.3 s with Steel HTTP 401 (invalid key). After the key was replaced: **Steel INT-2 passed** — run-b9af0c34e0 succeeded on the hosted catalog (20 checks passed, worker gpt-5.4 on a Steel session, 4 browser actions, 5 model calls, worker 22.1 s, 39.5 s total, records with hosted URLs) but the worker skipped the candidate and left its Steel session live; the account allows one concurrent session, so the next two attempts failed at session creation ("Could not initialize the browser session"). The leftover was released by hand; run-5b5d9ce59c then succeeded (20 checks, worker 22.8 s / 4 actions / 5 model calls, 40.4 s total), saved candidate demo-catalog.search_extract v1 in the hosted registry, and released its session (0 live sessions afterwards). The worker's Ghost decision is now persisted in the ARGUS report evidence (`evidence.ghost`) so a skipped candidate shows its reason. Tianqi to verify why the first hosted run's release did not complete.
  Steel qualification and reuse (same night): qualification by script with inputs {premium<=200, keyboard<=100, premium<=150 (empty)} succeeded — three replays with 0 model calls (9.9 s, 6.6 s, 8.2 s), all sessions released; demo-catalog.search_extract v1 is **qualified on the hosted registry**. Reuse through Mission Control on Steel: run-f4ba558e00 "headphones under 100 USD" succeeded — stage-4 preview "qualified workflow v1 compatible", worker_model ghost-semantic-replay, 0 worker model calls, worker 6.4 s, 21.3 s total, 20 checks passed, 0 live Steel sessions afterwards.
  Two Steel-only findings for Tianqi: (1) replaying a step that selects the "50 USD" option never observes value_equals on Steel (100/150/200 work; local Chrome handles 50), so the keyboard<=50 empty case fails qualification on Steel — reproduced three times, step trace shows results_ready False and applied parameters None after the select; (2) session creation intermittently fails in 0.4 s ("Could not initialize the browser session") when replays run back-to-back on the one-session account — a 5 s pause between replays avoided it once and not another time; the SDK error is swallowed by the adapter, so the worker should surface the status code. Abel changed `browser/adapter.py::settle` so a results_ready expectation waits for the page's ready signal (up to 5 s) instead of returning on the first content change; worker suite 94 passed; Tianqi to review.
Open issue / needed from / next action: P3-MOD landed (Codex, reviewed and patched by Abel; 22 tests). Steel path verified end to end (explore, qualify, reuse) on the hosted catalog; the concurrent-session limit (1) means a leaked or not-yet-freed session blocks the next run — release stale sessions before demos and avoid the 50 USD option in Steel demos until Tianqi fixes the select replay. Records carry no category field, so per-category selection needs the interpreter/planner to tag subtask records with their category (small controller or bridge change) before "cheapest per category" trims to one per category. Steel + hosted catalog not run (decision 6). Evidence images are not served (worker evidence refs are ids without files). Qualification needs the API route (P3-LEARN); tonight it ran by script. UI shows reuse only in the match reason and the report's worker_model; add a visible "replayed v4, 0 model calls" badge. Interpreter passes plural product words verbatim.
Receiver + connection check/result: none marked Integrated. Tianqi: verify worker_contracts against schemas.py and the operation/failure tables. Sting: verify decisions 1/2/9, the candidate references, and qualification-by-script. Thomas: review the moderator module rewrite and run a needs_visual continuation with UITARS_BASE_URL.
```
